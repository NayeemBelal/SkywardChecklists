...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (req.method) {
      case 'GET':
        // Get all floorplans for the zone
      ...
      case 'POST':
        ...
        const imagePath = await floorplanRepository.uploadImage(fileBuffer, originalFilename, mimeType, zoneId);

        // Save the record to database
        const newFloorplan = await floorplanRepository.create(zoneId, imagePath);
    ...
    console.error('Zone Floorplan API Error:', error);
    res.status(500).json({
