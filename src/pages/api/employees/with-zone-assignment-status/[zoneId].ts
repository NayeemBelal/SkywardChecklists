...
export default withAuth(async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!zoneId || Array.isArray(zoneId)) {
      res.status(400).json({ error: 'Invalid zoneId parameter' });
      return;
    }

    const zoneIdNum = parseInt(zoneId, 10);
    if (isNaN(zoneIdNum)) {
      res.status(400).json({ error: 'zoneId must be a number' });
      return;
    }
    ...
    const result = await repository.getEmployeesWithZoneAssignmentStatus(zoneIdNum, assignedPageNum, unassignedPageNum, pageSizeNum);

    res.status(200).json(result);
  } catch (error) {
    console.error('Employees with zone assignment status API Error:', error);
    res.status(500).json({ error: 'Failed to fetch employees with zone assignment status' });
  }
});
