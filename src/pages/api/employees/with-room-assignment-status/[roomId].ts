...
export default withAuth(async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await repository.getEmployeesWithRoomAssignmentStatus(roomIdNum, assignedPageNum, unassignedPageNum, pageSizeNum);

    res.status(200).json(result);
  } catch (error) {
    console.error('Employees with room assignment status API Error:', error);
    res.status(500).json({ error: 'Failed to fetch employees with room assignment status' });
  }
});
