...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { data: taskRows, error: taskErr } = await supabase
      .from('app_tasks')
      .select('id, description, description_es, task_description, task_description_es, room_id, frequency')
    ...
    const response = (taskRows || []).map((t: { id: number; description: string; description_es: string | null; task_description: string | null; task_description_es: string | null; room_id: number; frequency: string | null }) => {
      ...
      return {
        ...
        task_description_es: t.task_description_es,
